...
<main>
	...
	{:else if company}
		...
		{:else}
			...
			{:else if isLoadingScrapping}
				<div>
					<Loader />
				</div>
			{:else if scrapedContacts?.phoneNumbers?.length === 0 && scrapedContacts?.emails?.length === 0}
				<div class="mx-2">
					<p class="text-red">Nenhum contato encontrado</p>
				</div>
			{/if}
			...
			{#if company.domainOwner}
				<div class="mx-2 my-4">
					...
					<p>
						<b>{company.domainOwner?.documentType.toUpperCase()}: </b>{company.domainOwner
							?.document}
					</p>
